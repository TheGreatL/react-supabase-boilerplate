...
- **Data Processing (Pagination & Filtering)**:
    - ALWAYS implement server-side pagination (offset/cursor), filtering (WHERE clauses), and sorting (ORDER BY) for list endpoints.
    - NEVER return raw, unfiltered data dumps for the frontend to format. Validate and sanitize pagination parameters (`page`, `limit`) using Zod.
- **API Documentation**:
