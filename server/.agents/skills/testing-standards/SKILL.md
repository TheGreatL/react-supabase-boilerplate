...
description: This skill enforces robust testing methodologies for the backend. Use it when writing unit or integration tests using Vitest and Supertest.
---
...
- **API Testing**: Validate that REST API endpoints respond with the correct HTTP status codes and the strict `ApiResponse` JSON structure.
...
- **Database (Prisma) Strategy**: NEVER hit a production or shared development database.
  ...
  - *Option B (Preferred for E2E Tests)*: Spin up a fresh, isolated test database via Docker, run migrations, seed minimal data, and tear it down after.
